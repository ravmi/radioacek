...
namespace radioacek {
    Player::Player(string c_file, bool read_metadata) :
    ...
    void Player::findMaster(uint16_t listen_port) {
        controller_connection.serve(listen_port);
    }

    void Player::output(string message) {
        if(to_file)
            file_stream << message;
        else
            std::cout << message;
    ...
    void Player::init_response_correct(const string& server_answer) {
    ...
    string Player::send_init_message(string server_name, string path, uint16_t server_port) {
    ...
    void Player::connect(string server_name, string path, uint16_t server_port) {
