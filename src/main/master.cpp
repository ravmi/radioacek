...
const boost::regex play_pause_quit_title_regex("(?:(PAUSE|PLAY|QUIT|TITLE) (\\d{1,5}))\r\n|\n|\r\\0");
const boost::regex at_regex("AT (\\d{1,2})[.](\\d{1,2}) (\\d{1,9}+) (\\S+) (.* (\\S+) (\\d{1,5}) \\S+)\r\n|\n|\r\\0");
const boost::regex start_regex("START (\\S+) (.* (\\S+) (\\d{1,5}) \\S+)\r\n|\n|\r\\0");
    ...
    while (true) {
        try {
            ...
            if (prompt_message != "") {
                ...
                if (regex_match(prompt_message, result, play_pause_quit_title_regex)) {
                ...
                else if (boost::regex_match(prompt_message, result, at_regex)) {
                ...
                else if (regex_match(prompt_message, result, start_regex)) {
